...
class PerformanceMonitor {
  constructor() {
    ...
    this.isEnabled =
      config.isFeatureEnabled("performance") || config.isDebugMode();
      ...
      switch (name) {
        case "longTask":
          ...
          summary.metrics.longTasks = {
            ...
            averageDuration:
              longTasks.reduce((sum, v) => sum + v.duration, 0) /
                longTasks.length || 0,
        ...
        case "networkRequest":
          ...
          summary.metrics.network = {
            ...
            averageResponseTime:
              requests.reduce((sum, v) => sum + v.duration, 0) /
                requests.length || 0,
          };
          break;
    ...
    setInterval(() => {
      ...
      if (config.isFeatureEnabled("analytics")) {
        ...
        analytics.logEvent("performance_report", {
          ...
          network_success_rate: Math.round(
            summary.metrics.network?.successRate || 100,
          ),
          page_load_time: Math.round(
            summary.metrics.pageLoad?.loadComplete || 0,
          ),
        });
      }
