...
class Config {
  ...
  loadConfig() {
    const baseConfig = {
      app: {
        ...
        version: "1.0.0",
        description:
          "Il tuo personal trainer AI per allenamenti personalizzati",
  ...
  loadUserPreferences() {
    try {
      const userPrefs = localStorage.getItem(
  ...
  loadApiKeys() {
    this.config.apiKeys = {
      ...
      firebase: {
        ...
        authDomain: this.getApiKey(
          "FIREBASE_AUTH_DOMAIN",
          "fitnesspro.firebaseapp.com",
        ),
        projectId: this.getApiKey("FIREBASE_PROJECT_ID", "fitnesspro"),
        storageBucket: this.getApiKey(
          "FIREBASE_STORAGE_BUCKET",
          "fitnesspro.appspot.com",
        ),
  ...
  getApiKey(keyName, defaultValue) {
    ...
    try {
      const storedKey = localStorage.getItem(
        `${this.config.storage.prefix}apikey_${keyName.toLowerCase()}`,
      );
      ...
      console.warn(
        `Could not access localStorage for API key ${keyName}:`,
        error,
      );
    }
  ...
  validate() {
    ...
    if (
      maxWorkoutDuration &&
      (maxWorkoutDuration < 1 || maxWorkoutDuration > 480)
    ) {
